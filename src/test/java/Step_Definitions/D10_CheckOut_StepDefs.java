...
import Pages_POM_D_P.P10_CheckOut;
...
public class D10_CheckOut_StepDefs {
    P10_CheckOut checkOut = new P10_CheckOut();
   ...
   @And("Press on services terms and checkout button")
    ...
    public void data_of_billing_address_press_on_continue_button() throws InterruptedException {
        if (checkOut.continue_Btn_same_Address().isDisplayed()) {
            checkOut.continue_Btn_same_Address().click();
        } else {
            checkOut.checkout_firstName().click();
            checkOut.checkout_firstName().sendKeys("asmaa");

            checkOut.checkout_lastName().click();
            checkOut.checkout_lastName().sendKeys("ahmed");

            //checkOut.checkout_email().click();
            // checkOut.checkout_email().sendKeys("Tsaawt@example.com");

            Select select = new Select(checkOut.country());
            select.selectByValue("123");

            checkOut.checkout_city().click();
            checkOut.checkout_city().sendKeys("Giza");

            checkOut.checkout_address().click();
            checkOut.checkout_address().sendKeys("4 St. Giza");

            checkOut.checkout_postalCode().click();
            checkOut.checkout_postalCode().sendKeys("02");

            checkOut.checkout_phone().click();
            checkOut.checkout_phone().sendKeys("01111555789");
